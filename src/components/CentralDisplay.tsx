...
export const CentralDisplay: Component<CentralDisplayProps> = (props) => {
    ...
    return (
        <div ref={element} class="central-display" classList={{visible: !!props.children}} onClick={props.hideCentralDisplay}>
            <div class="cd_child">
                {props.children}
            </div>

            <button type='button' onClick={() => console.log('test')}> CLOSE </button>
        </div>
    )
}
